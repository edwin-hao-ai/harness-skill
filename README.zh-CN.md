...
> 两端都默认走 **agent + AI**：由 agent 在对话里提问、由 agent 用自己的判断撰写文档。
> 仓库另带一套**确定性脚本**作为 CI/测试与无法维持对话状态时的兜底。
...
> **`spec.md` / `design.md` / `architecture.md` 别写串**：`spec.md` 是产品需求（做什么、为什么）；
> `design.md` 是工程设计文档，作用是**约束实现**（备选方案、哪些是硬约束）；
> `architecture.md` 是落定后可速查的架构参考。

## 兜底脚本（CI / 无对话状态时）

这些是**规则化**（模板替换），内容深度不如 agent 主路径，仅用于终端/CI 或运行时无法维持状态时：
