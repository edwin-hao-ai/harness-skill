...
| `design.md` | 视觉长什么样、设计令牌、样式准则 | **YAML 设计令牌**（colors/typography/spacing/rounded/components）、品牌概述、颜色系统、排版系统、布局间距、层级深度、形状、组件样式、Do's and Don'ts | 产品需求（放 spec.md）、技术架构（放 architecture.md） |
| `architecture.md` | 最终长什么样 | 技术栈、组件/模块图、数据模型、部署、集成（可读速查） | "为什么"的长篇论证、视觉设计 |

> design.md 遵循 **Google DESIGN.md 规范**（2026年4月开源），是**视觉设计系统文档**：
> 通过 YAML front matter 定义机器可读的设计令牌（颜色、排版、间距、圆角、组件），
> 通过 Markdown 正文提供人类可读的设计理念和样式指南。
> 
> **核心价值**：让 AI 编码代理能够持久、结构化地理解项目的视觉设计系统。
